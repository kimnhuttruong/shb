...
class PrimaryBtn extends StatelessWidget {
  const PrimaryBtn(
      {Key? key, required this.onPressed, this.label, this.icon, this.child, this.leadingIcon = false, this.isCompact = false, this.cornerRadius})
      : super(key: key);
  final Widget? child;
  final String? label;
  final IconData? icon;
...
class SimpleBtn extends StatelessWidget {
  const SimpleBtn({Key? key, this.onPressed, required this.child, this.focusMargin, this.normalColors, this.hoverColors, this.cornerRadius})
      : super(key: key);
  final Widget child;
  final VoidCallback? onPressed;
...
class TextBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    TextStyle finalStyle =
        style ?? TextStyles.caption.copyWith(decoration: showUnderline ? TextDecoration.underline : TextDecoration.none, fontWeight: FontWeight.w500);
